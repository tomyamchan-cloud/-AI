...
# New function content as a raw string to avoid character issues
NEW_FUNCTION = r"""function buildSystemPrompt(conversationHistory) {
...
with open(r'c:\授業づくりAI\main.js', 'r', encoding='utf-8', errors='ignore') as f:
    content = f.read()

# Replace the entire buildSystemPrompt function
pattern = re.compile(r'function buildSystemPrompt\(conversationHistory\) \{.*?^}', re.DOTALL | re.MULTILINE)
new_content = pattern.sub(NEW_FUNCTION, content)

with open(r'c:\授業づくりAI\main.js', 'w', encoding='utf-8') as f:
    f.write(new_content)
